...
# 检测是否通过 curl 管道执行（/dev/fd/XX 是临时文件描述符）
if [[ "$SCRIPT_DIR" == /dev/fd/* ]]; then
  echo -e "${RED}✗ 检测到通过 curl 管道执行，无法获取项目根目录${NC}"
  echo ""
  echo "  请使用以下任一方式安装："
  echo ""
  echo "  ${BOLD}方式一（推荐）：先克隆仓库${NC}"
  echo "  git clone https://github.com/wanikua/danghuangshang.git"
  echo "  cd danghuangshang"
  echo "  bash install.sh"
  echo ""
  echo "  ${BOLD}方式二：使用完整安装脚本${NC}"
  echo "  bash <(curl -fsSL https://raw.githubusercontent.com/wanikua/danghuangshang/main/scripts/full-install.sh)"
  echo ""
  exit 1
fi
